...
public class AddEditReservationFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        binding.editTextReservationName.addTextChangedListener(new OnTextChangeListener() {
            @Override
            public void onTextChange(String text) {
                viewModel.onNameChange(text);
            }
        });
        binding.editTextReservationPhone.addTextChangedListener(new OnTextChangeListener() {
            @Override
            public void onTextChange(String text) {
        ...
        binding.numberPickerPersonNumber.setMaxValue(30);
        ...
        viewModel.getEvent().observe(getViewLifecycleOwner(), event -> {
            if (event instanceof AddEditReservationViewModel.Event.ShowGeneralMessage) {
                ...
                showTimeDialog();
            } else if (event instanceof AddEditReservationViewModel.Event.FillNameAndPhone) {
                String name = ((AddEditReservationViewModel.Event.FillNameAndPhone) event).name;
                String phone = ((AddEditReservationViewModel.Event.FillNameAndPhone) event).phone;
                binding.editTextReservationName.setText(name);
                binding.editTextReservationPhone.setText(phone);
            }
        });
    }
