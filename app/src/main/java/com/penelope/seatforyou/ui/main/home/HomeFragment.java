...
public class HomeFragment extends Fragment implements OnMapReadyCallback, NaverMap.OnLocationChangeListener {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        binding.textViewRecomShop.setOnClickListener(v -> viewModel.onRecommendedShopClick());
        ...
        binding.recyclerRealReview.setAdapter(shopAdapter);

        shopAdapter.setOnItemSelectedListener(position -> {
            Shop shop = shopAdapter.getCurrentList().get(position);
            viewModel.onRealReviewShopClick(shop);
        });

        viewModel.getRealReviewShops().observe(getViewLifecycleOwner(), shops -> {
            if (shops != null) {
        ...
        viewModel.getRecommendedShop().observe(getViewLifecycleOwner(), shop -> {
            if (shop != null) {
                binding.textViewRecomShopName.setText(shop.getName());
                Glide.with(this)
                        .load(ImageUtils.getShopImageUrl(shop.getUid()))
                        .transition(DrawableTransitionOptions.withCrossFade())
                        .into(binding.imageViewRecomShop);
            } else {
                String title = "Seat for You";
                binding.textViewRecomShopName.setText(title);
            }
        });
        ...
        viewModel.getEvent().observe(getViewLifecycleOwner(), event -> {
            if (event instanceof HomeViewModel.Event.NavigateToDetailScreen) {
                ...
                boolean showReview = ((HomeViewModel.Event.NavigateToDetailScreen) event).showReview;
                NavDirections navDirections = HomeFragmentDirections.actionGlobalDetailFragment(shop, showReview);
    ...
    public void onResume() {
        ...
        timer = new CountDownTimer(86400 * 1000, 3000) {
            @Override
