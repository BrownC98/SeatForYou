...
public class RegisterViewModel extends ViewModel {
    ...
    public void onRegisterClick() {
        ...
        if (password.length() < 8) {
            event.setValue(new Event.ShowGeneralMessage("비밀번호를 8자리 이상 입력하세요"));
            return;
        }
        ...
        if (AuthUtils.isSequential(password)) {
            event.setValue(new Event.ShowGeneralMessage("연속하는 숫자를 비밀번호로 사용할 수 없습니다"));
            return;
        }

        if (!AuthUtils.hasAnySpecial(password)) {
            event.setValue(new Event.ShowGeneralMessage("비밀번호는 1개 이상의 특수문자를 포함해야 합니다"));
            return;
        }
