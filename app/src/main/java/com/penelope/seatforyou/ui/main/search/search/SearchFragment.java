    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        viewModel.getEvent().observe(getViewLifecycleOwner(), event -> {
            if (event instanceof SearchViewModel.Event.ShowGeneralMessage) {
                ...
                int minPrice = ((SearchViewModel.Event.NavigateToSetFilterScreen) event).minPrice;
                int maxPrice = ((SearchViewModel.Event.NavigateToSetFilterScreen) event).maxPrice;
                List<String> categories = ((SearchViewModel.Event.NavigateToSetFilterScreen) event).categories;
                NavDirections navDirections = SearchFragmentDirections.actionSearchFragmentToSetFilterFragment(
                        region, minPrice, maxPrice, new ArrayList<>(categories));
        ...
        getParentFragmentManager().setFragmentResultListener("set_filter_fragment", getViewLifecycleOwner(),
                (requestKey, result) -> {
                    ...
                    int minPrice = result.getInt("min_price");
                    int maxPrice = result.getInt("max_price");
                    List<String> categories = result.getStringArrayList("categories");
                    viewModel.onFilterResult(region, minPrice, maxPrice, categories);
                });
    }
