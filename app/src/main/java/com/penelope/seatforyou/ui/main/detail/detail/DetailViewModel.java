...
public class DetailViewModel extends ViewModel {
    ...
    public void onZzimClick() {
        if (uid != null) {
            userRepository.addRemoveDib(uid, shop.getUid(),
                    addedRemoved -> {
                        if (addedRemoved) {
                            event.setValue(new Event.ShowGeneralMessage("찜 목록에 추가되었습니다"));
                        } else {
                            event.setValue(new Event.ShowGeneralMessage("찜 목록에서 삭제되었습니다"));
                        }
                    },
